...
#Enter glycolysis and oxphos gene sets
metabolic_gene_sets <- read.table(file = "Human Core Glycolysis OxPhos Hypoxia gene sets.txt", header = T, sep = "\t")
# Enter your background gene sets
all_pathways <- read.table(file = "KEGG_Metabolic_pathways.txt", sep = "\t", header = T)

# change directory for output files
setwd("/Users/JamesJoly/Documents/Research/Bioinformatics/Differential\ Pathway\ GSEA/GSEA\ on\ Corr\ bt\ DPEA\ Pathway\ to\ DepMap/")
output.file.name <- "Differential Core Glycolysis OxPhos - DepMap Correlations Glyc OxPhos DPEA.csv"
nperm = 1000
...
#Hypoxia
for (i in 1:nrow(data_in)){
  if (data_in$Gene[i] %in% metabolic_gene_sets$Hypoxia.Pathway){
    data_in$Hypoxia.Pathway[i] = "X";
  }
}
...
# Results.All.Cells <- matrix(data = NA, nrow = length(cells), ncol = 20)
# colnames(Results.All.Cells) <- c("Cell","Glycolysis ES","Glycolysis NES","Glycolysis p-value", "Glycolysis FDR corrected p-value - BH", "Glycolysis FDR corrected p-value - NES",
#                                  "OxPhos ES","OxPhos NES","OxPhos p-value", "OxPhos FDR corrected p-value - BH","OxPhos FDR corrected p-value - NES",
#                                  "DPEA ES","DPEA NES", "DPEA p-value", "DPEA FDR corrected p-value - BH","DPEA FDR corrected p-value - NES",
#                                  "Hypoxia ES","Hypoxia NES","Hypoxia p-value", "Hypoxia FDR corrected p-value")
# Results.All.Cells <- as.data.frame(Results.All.Cells)
# Results.All.Cells$Cell <- cells

Results.All.Cells <- matrix(data = NA, nrow = 0, ncol = 8)
colnames(Results.All.Cells) <- c("Cell","Pathway","KS","KS_Normalized",
                                 ...
                                 "FDR corrected p-value - BH", "FDR corrected p-value - NES")
...
###### Calculate KS scores for real data for Pathway 1 and Pathway 2 and the difference
#  data_in2 <- cbind(select(data_in, ends_with("Pathway")),
#                    select(data_in, cells[1])) 


## Assuming first two columns in data table are Genes and Rank Metric (e.g. Foldchange, SNR)

Results <- matrix(data = NA, nrow = length(combined_pathways), ncol = 8)
...
# KSRandomArray <- matrix(data = NA, nrow  = nperm, ncol = length(all_pathways_unique))
# colnames(KSRandomArray) <- all_pathways_unique
...
#KSRandomArray <- as.data.frame(matrix(data = NA, nrow = nperm, ncol = length(combined_pathways)))
KSRandomArray <- list()
...
KSRandomArray$DPEA <- KSRandomArray$Glycolysis.Pathway - KSRandomArray$OxPhos.Pathway

KSRandomArray.Norm <- as.data.frame(KSRandomArray.Norm)
pos.DPEA.perms <- KSRandomArray[which(KSRandomArray$DPEA >= 0),]$DPEA
neg.DPEA.perms <- KSRandomArray[which(KSRandomArray$DPEA < 0),]$DPEA

norm.pos.DPEA.perms <- pos.DPEA.perms / mean(pos.DPEA.perms)
norm.neg.DPEA.perms <- neg.DPEA.perms / mean(neg.DPEA.perms) * -1

norm.DPEA.perms <- c(norm.pos.DPEA.perms,norm.neg.DPEA.perms)

KSRandomArray.Norm$DPEA <- norm.DPEA.perms


###Make a DPEA KSRandomArray for Glyc - All Pathways Unique
DPEA_RandomArray_A <- matrix(data = NA, nrow = nrow(KSRandomArray), ncol = length(all_pathways_unique))
DPEA_RandomArray_A <- as.data.frame(DPEA_RandomArray_A)
DPEA_RandomArray_B <- matrix(data = NA, nrow = nrow(KSRandomArray), ncol = length(all_pathways_unique))
DPEA_RandomArray_B <- as.data.frame(DPEA_RandomArray_B)

DPEA_Real_A <- as.data.frame(matrix(data = NA, nrow = 1, ncol = length(all_pathways_unique)))

DPEA_Real_B <- as.data.frame(matrix(data = NA, nrow = 1, ncol = length(all_pathways_unique)))


for (i in 1:length(all_pathways_unique)){
  identifier <- paste("DPEA A", all_pathways_unique[i])
  colnames(DPEA_RandomArray_A)[i] <- identifier
  DPEA_RandomArray_A[,i] <- KSRandomArray[,"Glycolysis.Pathway"] - KSRandomArray[,all_pathways_unique[i]]
  colnames(DPEA_Real_A)[i] <- identifier
  DPEA_Real_A[,i] <- Results[Results$Pathway == "Glycolysis.Pathway",]$KS -Results[Results$Pathway == all_pathways_unique[i],]$KS
  
  identifier <- paste("DPEA B", all_pathways_unique[i])
  colnames(DPEA_RandomArray_B)[i] <- identifier
  DPEA_RandomArray_B[,i] <- KSRandomArray[,"OxPhos.Pathway"] - KSRandomArray[,all_pathways_unique[i]]
  colnames(DPEA_Real_B)[i] <- identifier
  DPEA_Real_B[,i] <-  Results[Results$Pathway == all_pathways_unique[i],]$KS - Results[Results$Pathway == "OxPhos.Pathway",]$KS
}

DPEA_Real <- cbind(DPEA_Real_A,DPEA_Real_B)
DPEA_RandomArray <- cbind(DPEA_RandomArray_A, DPEA_RandomArray_B)

#normalize the DPEA distribution
DEEPS <- colnames(DPEA_RandomArray)
...
for (i in 1:length(DEEPS)){
  ...
  DPEA_RandomArray.temp <- DPEA_RandomArray[,DEEPS[i]]
  pos.temp <- DPEA_RandomArray.temp[which(DPEA_RandomArray.temp >= 0)]
  neg.temp <- DPEA_RandomArray.temp[which(DPEA_RandomArray.temp < 0)]
...
#Now we have a distribution of DPEA NES across all pathways and a distribution of Real DPEA NES
#We can use this for FDR calcs.


DPEA <- matrix(data = NA, nrow = 1, ncol = 8)
colnames(DPEA) <- c("Cell","Pathway","KS","KS_Normalized",
                    "Frequency of Random KS Scores > KS Score observed","Position at Max",
                    "FDR corrected p-value - BH", "FDR corrected p-value - NES")
DPEA <- as.data.frame(DPEA)
DPEA$Pathway <- c('DPEA')
DPEA$Cell <- cells[u]

DPEA$KS<- Results[Results$Pathway == "Glycolysis.Pathway",]$KS - Results[Results$Pathway == "OxPhos.Pathway",]$KS
Results <- rbind(DPEA,Results)
...
Total.NES.DPEA <- as.vector(DEEPS.norm)
...
### Change Total.NES.DPEA to the permutation NES.DPEA for A-B
if (Results[Results$Pathway == "DPEA",]$KS > 0){
  temp <- sum(norm.pos.DPEA.perms > Results[Results$Pathway == "DPEA",]$KS_Normalized)
  percent.temp <- temp / length(norm.pos.DPEA.perms)
  if (percent.temp / percent.real.NES.pos < 1){
    Results[Results$Pathway == "DPEA",]$`FDR corrected p-value - NES` <- percent.temp / percent.real.NES.pos
  } else {Results[Results$Pathway == "DPEA",]$`FDR corrected p-value - NES` = 1}
} else if (Results[Results$Pathway == "DPEA",]$KS < 0){
  temp <- sum(norm.neg.DPEA.perms < Results[Results$Pathway == "DPEA",]$KS_Normalized)
  percent.temp <- temp / length(norm.neg.DPEA.perms)
  if (percent.temp / percent.real.NES.neg < 1){
    Results[Results$Pathway == "DPEA",]$`FDR corrected p-value - NES` <- percent.temp / percent.real.NES.neg
  } else {Results[Results$Pathway == "DPEA",]$`FDR corrected p-value - NES` = 1}
}

Total.NES.GSEA <- as.vector(KSRandomArray.Norm)

#Start at 2 b/c DPEA is pathways2[1]
...
for(i in 2:length(pathways2)){
  ...
  if (Results[Results$Pathway == pathways2[i],]$KS > 0){
    ...
    if (percent.temp / percent.real.NES.pos < 1){
      Results[Results$Pathway == pathways2[i],]$`FDR corrected p-value - NES` <- percent.temp / percent.real.NES.pos
      } else {Results[Results$Pathway == pathways2[i],]$`FDR corrected p-value - NES` = 1}
    } 
  else if (Results[Results$Pathway == pathways2[i],]$KS < 0){
    ...
    if (percent.temp /percent.real.NES.neg < 1){
      Results[Results$Pathway == pathways2[i],]$`FDR corrected p-value - NES` <- percent.temp / percent.real.NES.neg
    } else {Results[Results$Pathway == pathways2[i],]$`FDR corrected p-value - NES` = 1}
  }
}
...
# Results.All.Cells[Results.All.Cells$Cell == cells[u],]$`Glycolysis ES` = Results[Results$Pathway =="Glycolysis.Pathway",]$KS
# Results.All.Cells[Results.All.Cells$Cell == cells[u],]$`Glycolysis NES` = Results[Results$Pathway =="Glycolysis.Pathway",]$KS_Normalized
# Results.All.Cells[Results.All.Cells$Cell == cells[u],]$`Glycolysis p-value` = Results[Results$Pathway =="Glycolysis.Pathway",]$`Frequency of Random KS Scores > KS Score observed`
# 
# Results.All.Cells[Results.All.Cells$Cell == cells[u],]$`OxPhos ES` = Results[Results$Pathway == "OxPhos.Pathway",]$KS
# Results.All.Cells[Results.All.Cells$Cell == cells[u],]$`OxPhos NES` = Results[Results$Pathway == "OxPhos.Pathway",]$KS_Normalized
# Results.All.Cells[Results.All.Cells$Cell == cells[u],]$`OxPhos p-value` = Results[Results$Pathway == "OxPhos.Pathway",]$`Frequency of Random KS Scores > KS Score observed`
# 
# Results.All.Cells[Results.All.Cells$Cell == cells[u],]$`DPEA ES` = Results[Results$Pathway =="DPEA",]$KS
# Results.All.Cells[Results.All.Cells$Cell == cells[u],]$`DPEA NES` = Results[Results$Pathway =="DPEA",]$KS_Normalized
# Results.All.Cells[Results.All.Cells$Cell == cells[u],]$`DPEA p-value` = Results[Results$Pathway =="DPEA",]$`Frequency of Random KS Scores > KS Score observed`
# Results.All.Cells[Results.All.Cells$Cell == cells[u],]$`DPEA FDR corrected p-value - NES` = Results[Results$Pathway =="DPEA",]$`FDR corrected p-value - NES`
# 
# 
# Results.All.Cells[Results.All.Cells$Cell == cells[u],]$`Hypoxia ES` = Results[Results$Pathway == "Hypoxia.Pathway",]$KS
# Results.All.Cells[Results.All.Cells$Cell == cells[u],]$`Hypoxia NES` = Results[Results$Pathway == "Hypoxia.Pathway",]$KS_Normalized
# Results.All.Cells[Results.All.Cells$Cell == cells[u],]$`Hypoxia p-value` = Results[Results$Pathway == "Hypoxia.Pathway",]$`Frequency of Random KS Scores > KS Score observed`
...
if (Make.Plots == TRUE){
  p <- {
    ...
    gsea.enrichment.score <- Results[Results$Pathway == "DPEA",]$KS
    gsea.normalized.enrichment.score <- Results[Results$Pathway == "DPEA",]$KS_Normalized
    gsea.p.value <- signif(Results[Results$Pathway == "DPEA",]$`Frequency of Random KS Scores > KS Score observed`, digits = 3)
    ...
    ### Rank metric plotting
    #par(mar = c(0, 5, 0, 2))
    
    # rank.colors <- data_in2[,cells[u]] - metric.range[1]
    ...
    # barplot(matrix(rank.colors$lengths), col = rank.colors$values, border = NA, horiz = TRUE, xaxt = "n", xlim = c(1, length(data_in2[,cells[u]])))
    #box()
    # text(length(data_in2[,cells[u]]) / 2, 0.7,
    #      labels = ifelse(!missing(class.name), class.name, gsea.template))
    # text(length(data_in2[,cells[u]]) * 0.01, 0.7, "Positive", adj = c(0, NA))
    # text(length(data_in2[,cells[u]]) * 0.99, 0.7, "Negative", adj = c(1, NA))
    ...
    barplot(rank.metric$values, col = "lightgrey", lwd = 0.1, xaxs = "i",
            ...
            space = 0, add = TRUE)
    box()
    dev.print(pdf,file = paste(cells[u],"DPEA.pdf"), height = 4, width = 4)
    dev.off()
  }
  par(def.par)
}
...
#FDR Benjaminini-Hochburg correction 
Results.All.Cells <- as.data.frame(Results.All.Cells) 
Results.All.Cells$`FDR corrected p-value - BH` <- p.adjust(Results.All.Cells$`Frequency of Random KS Scores > KS Score observed`, 
                                                           method = "BH", n = length(Results.All.Cells$`Frequency of Random KS Scores > KS Score observed`))
...
DPEA.Results <- Results.All.Cells[Results.All.Cells$Pathway == "DPEA",]

#uncomment these lines if you want GSEA results across all gene sets
#GSEA.Results <- Results.All.Cells[Results.All.Cells$Pathway != "DPEA",]
#write.csv(GSEA.Results, file = paste("GSEA Only", output.file.name), row.names = FALSE)

### DPEA Comparisons have Glyc, OxPhos, and DPEA
DPEA.Results <- Results.All.Cells[Results.All.Cells$Pathway == "DPEA" |
                                    Results.All.Cells$Pathway == "Glycolysis.Pathway" | 
                                    Results.All.Cells$Pathway == "OxPhos.Pathway",]
write.csv(DPEA.Results, file = paste("DPEA Results", output.file.name), row.names = FALSE)
